...
The key technique for attributing call sites to their containing function is **byte-range containment**: we binary-search the sorted list of function definitions by `start_byte` to find which function contains each call expression. O(log n) per call site, no extra tree traversal needed.
