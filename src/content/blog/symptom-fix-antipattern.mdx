...
## Failure Point 5: RAG Is for Similarity, Not Structure

RAG finds semantically similar content — it's excellent for locating files about related topics, similar error patterns, similar domains. It is not designed to return every return statement in a function. Return statements in the same function aren't semantically similar to each other. They're structurally related. RAG doesn't know about structure.

For code *discovery*, RAG is the right tool. For code *auditing* — reading every path through a function — you need the full file.

With the right file identified and the state handoff fixed, `DiagnosisAgent` was pointing at `runPrankChecker` in `prankCheckerMain.js`. The fix agent audited the function and concluded it was fine.
