...
*Part 9 of the Code RAG series. Parts 1–8 built a pipeline that indexes correctly, retrieves relevantly, and rebuilds safely. This post covers the failure mode that none of that prevents: answers that are wrong in ways you cannot see.*
...
*This post completes the nine-part Code RAG series. Starting from a line-count chunker that split functions across arbitrary windows, we built: structural diagnosis of two failure modes, function-boundary extraction, LLM-generated descriptions, hybrid search, a document registry, three-way decision logic for re-chunking vs re-embedding vs re-indexing, alias-based zero-downtime rebuilds, and end-to-end retrieval tracing. Every change was measured against real code from a production codebase.*
