...
seriesPart: 1
seeAlso: ["rag-boilerplate-dominance"]
---

*A new series alongside the Incident RAG posts. Same production codebase — AllInterviews, a Node.js/Express backend with 422 JS/TS files. Different problem: indexing source code for semantic search so agents can find relevant code when diagnosing and fixing bugs.*

---

## Why code RAG is different from incident RAG

The incident RAG series indexed short, dense documents — each one a formatted string like:

```
S3_NO_SUCH_KEY: S3 throws NoSuchKey on missing key | Root cause: The copyObject call fails when the source key doesn't exist before the move completes
```

Every document was purpose-built for retrieval. One incident, one text, written in natural language about what went wrong.
