...
1. **Function-boundary chunks** — one chunk per function, sized to the function. Eliminates dilution. This is the high-leverage fix and the prerequisite for everything else.
2. **LLM-generated descriptions** — appended to each chunk at index time. Closes the vocabulary gap partially by adding error names and API dependencies the source code never mentions.
