...
seriesPart: 1
seeAlso: ["rag-index-vs-live-store"]
---
...
The same `TypeError: Cannot read properties of undefined (reading 'publish_decision')` created a brand new incident on the production cluster, ran full triage, diagnosis, and fix generation — complete API cost, complete pipeline — despite an existing open incident already having a PR open for the exact same error.
...
```
Layer 1  →  Postgres query for non-terminal incidents   — hard block
Layer 2  →  Postgres query for resolved incidents       — soft: regression context
Layer 3  →  RAG semantic search (pgvector)              — soft: context for TriageAgent
```
...
## Failure Point 1: Whitespace Broke the Description Match

Layer 1 queries Postgres for all non-terminal incidents, then compares the incoming error description against each one using a normalization function:
...
To understand why, the distinction between Layer 2 and Layer 3 matters:

- **Layer 2** queries for *resolved* incidents — past errors that were fixed and closed. Its job is regression detection: if the same error type was resolved before and is now re-occurring, inject that context into the triage prompt.
- **Layer 3** (RAG) searches for *open* incidents — currently active ones. Its job is duplicate detection: if there's already an open incident with a PR, drop the incoming event entirely.

These are two different questions about two different populations. The bug was that the code treated an answer to the first question as a reason to skip the second.

```python
# Layer 2 — queries resolved incidents
...
There was a previously resolved incident for the same error type and service. Layer 2 found it, correctly set `prior_context`, and moved on. But that `if prior_context is None` guard meant Layer 3 never ran — and Layer 3 was the only check that would have found the currently open incident with an active PR.

Layer 2 did its job correctly. The problem was the implicit assumption baked into the guard: that "Layer 2 found regression context" and "Layer 3 might find an open duplicate" are mutually exclusive. They aren't. An error can have a prior resolved incident *and* a currently open PR at the same time — which is exactly the situation here.
...
- **Live store lookup**: ignores pgvector metadata, reads current status directly from Postgres
- **Higher threshold (0.90 vs 0.80)**: hard blocks should only fire on near-identical matches

---

## Failure Point 4: Vector Index Metadata Goes Stale

The live store lookup in the fix above is deliberate. pgvector stores `status` and `pr_url` as metadata at index time and doesn't update them as the incident moves through its lifecycle. Using `s["status"]` from the search result would mean making a blocking decision based on a snapshot that could be many state transitions out of date.

The full explanation of why this is a category error — and the general rule for any vector index sitting in front of a live data store — is in [RAG Finds the Candidate. The Live Store Confirms the Truth.](/blog/rag-index-vs-live-store).
...
<Mermaid
  chart={`flowchart TB
    Event([Incoming error event]) --> L1{Layer 1<br/>Postgres query<br/>+ normalized match}
    L1 -- match --> Drop1([drop · zero API cost])
    L1 -- miss --> L2[Layer 2<br/>Postgres resolved lookup]
    L2 -- regression found --> Ctx1[Soft: regression context<br/>for TriageAgent]
    L2 -- nothing --> L3
    Ctx1 --> L3
    L3{Layer 3<br/>RAG semantic ≥ 0.90<br/>unconditional}
    L3 -- candidates --> Live[(Live store<br/>lookup)]
    Live -- open + has PR --> Drop2([drop · 1 embedding call])
    Live -- terminal/no PR --> L3b
    L3 -- no candidates --> L3b
    L3b[Layer 3b<br/>RAG context<br/>soft hint]
    L3b --> Triage([TriageAgent])
    classDef event fill:#0b0d10,stroke:#2f343b,color:#e6e8eb
    classDef hard fill:#0b0d10,stroke:#6ee7b7,color:#e6e8eb
    classDef soft fill:#13161a,stroke:#23272d,color:#9aa1a9
    class Event,Drop1,Drop2,Triage event
    class L1,L3 hard
    class L2,L3b,Ctx1,Live soft`}
  caption="The dedup pipeline after all four fixes. Layer 1 catches the common case with zero API cost. Layer 3 runs unconditionally and reads Postgres directly — it cannot be fooled by stale vector index metadata."
/>
...
**RAG metadata goes stale.** The vector index stores what you give it at index time. If the underlying record changes after indexing, the metadata is wrong. For any decision that depends on current state — is this incident still open? does this PR still exist? — query Postgres directly. Use RAG to find candidates, not to confirm current truth.
